...
pub(crate) fn pan_orbit_camera(
    ...
    for (settings, mut state, mut transform) in &mut q_camera {
		...
		let mut movement_total = Vec3::ZERO;

		if settings.forward_key.map(|key| kbd.pressed(key)).unwrap_or(false) {
            movement_total.z -= 0.1;
        }
		if settings.back_key.map(|key| kbd.pressed(key)).unwrap_or(false) {
            movement_total.z += 0.1;
        }
		if settings.left_key.map(|key| kbd.pressed(key)).unwrap_or(false) {
            movement_total.x -= 0.1;
        }
		if settings.right_key.map(|key| kbd.pressed(key)).unwrap_or(false) {
            movement_total.x += 0.1;
        }

		if movement_total != Vec3::ZERO {
            any = true;
			movement_total = transform.rotation * movement_total;
			//movement_total.z += movement_total.y;
			movement_total.y = 0.0;
		}
